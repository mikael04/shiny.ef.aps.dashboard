...
app_server <- function(input, output, session) {
  ...
  newcase <- T
  ### Resultado dos cálculos de novos gráficos (uma lista que deve ser composta)
  result_graphs <- NULL
  ## 0.1.1 Debug ----
  debug <- F
  read_data_rds <- F
  ...
  ## 2.0 Reatividade Gráficos Iniciais
  ## Para otimizar a geração dos gráficos, foi feito as versões iniciais
  ## junto com versões reativas do seletor
  ...
  ## 2.2 Títulos iniciais de mapa e gráfico de eficiência ----
  ...
  ## 2.3 Caixa do gráfico de eficiência ----
  output$box_graf_ef <- renderUI({
    div(
      id = "graf-ef",
      class="row",
  ...
  ### 3.1 Criando promise para aplicar filtros ----
  ## Usando Extended Task do Shiny
  result_func <- ExtendedTask$new(function(
  ...
  observeEvent(input$applyFilters, {
    ...
    if(is.null(promise)){
      ...
      ## Se estiver em modo promises,
      if(!debug){
        browser()
        ### 3.1.1. Chamando promise (extended task) ----
      ...
      ## Se estiver em modo processo único do Shiny
      if(debug){
        ## 3.2 Usando único processo Shiny ----
        ...
        ### 3.2.1. Nova tabela de eficiência ----
        ...
        # browser()
        ### 3.2.3 Novo gráfico de Inputs e outputs ----
        ### Inputs e outputs
        mod_graph_lollipop_inputs_outputs_server("graph_lollipop_inputs_outputs_1",
  ...
  ## 3.1.2 Usando o resultado da promise ----
  result_graphs <- reactive({
    ...
    ## Tabela
    gt_tabela <- result[[1]][[1]]
    ## Mapa
    ggiraph_map <- result[[2]][[1]]
    ## Eficiência
    list_graphs_inputs_outputs <- result[[3]][[1]]

    ### Nova tabela de eficiência ----
    mod_tabela_ef_server("tabela_ef_1", F, gt_tabela)
    ...
    # browser()
    # Novo gráfico de Inputs e outputs ----
    ### Inputs e outputs
    mod_graph_lollipop_inputs_outputs_server("graph_lollipop_inputs_outputs_1",
                                             initial_state = F, ef_proc_res,
                                             list_graphs_inputs_outputs, newcase)
