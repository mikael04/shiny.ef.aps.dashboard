#' aux_graph_lollipop_input_res_1
#'
#' @description Função para lidar com tipos de gráficos e gerar gráfico de resultados do tipo pirulito para entrada 1
#'
#' @return Retornará um gráficos de entrada que servirá para os inputs no patchwork
#'
#' @noRd

func_aux_graph_lollipop_input_res_1 <- function(
  ...
  # browser()
  ## Nomes dos índices
  input_names <- c("Despesas em APS \n por hab coberto (em R$)", "Equipe da APS \npor 3.500 hab cob")
  ...
  if(graph_type == 2){
    ...
    i <- 2
    ###  Quantas colunas terá que pular para ter o valor que falta para eficiência
    ### (como é apenas uma variável, é a próxima)
    cols_jump <- 1
    # browser()
